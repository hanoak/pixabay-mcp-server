...
describe('nodeVersionError', () => {
  ...
  it('does not block on an unrecognized version string', () => {
...
describe('MIN_NODE_MAJOR', () => {
  it('stays in sync with package.json engines.node', () => {
    // package.json is the documented source of truth (and the range npm enforces
    // at install). The guard mirrors it; this asserts the mirror is accurate so
